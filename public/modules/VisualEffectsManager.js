// Visual Effects system for EvoChess
// Handles particle effects, animations, transitions, and visual feedback
...
class VisualEffectsManager {
  ...
  createParticle() {
    if (typeof THREE === 'undefined') {
      console.warn('THREE.js not available for particle creation');
      return null;
    }
  ...
  animatePieceMovement(piece, fromPos, toPos, duration = 1000) {
    if (typeof THREE === 'undefined') {
      console.warn('THREE.js not available for piece animation');
      return;
    }
    
    const mesh = typeof pieceMeshes !== 'undefined' ? pieceMeshes[piece.id] : null;
    if (!mesh) return;
  ...
  cleanup() {
    ...
    this.particleSystem.pool.length = 0;
    this.particleSystem.active.length = 0;
  }
}

// Standalone utility functions for visual effects
